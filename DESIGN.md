...
The cursor deliberately does not include `chainId`. If we go multi-chain, `chainId` becomes a required query parameter — cleaner than smuggling it into the cursor. Cross-chain pagination for one integrator isn't a use case we need.
