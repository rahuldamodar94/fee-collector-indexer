...
**Finality-anchored indexing with panic halt on reorg.** Safe head is `finalized` where available, otherwise `latest - CONFIRMATION_DEPTH`. The scanner never reads past it. Integrators consume this data for fee accounting, not a live UI, so a few minutes of lag is fine and wrong numbers are not. Correctness over freshness, on purpose. Rolling back on reorg would be a whole code path for something the finalized tag already prevents, so it stays out. As a backstop, at the start of each scan cycle the scanner fetches the first new block (`fromBlock`) and compares its `parentHash` to the hash of the previously stored last block. One check per cycle, at the boundary, not per block inside the chunk. A mismatch means the chain reorged between cycles; the scanner halts that chain, increments `fee_collector_reorg_detected_total`, and exits.
...
The cursor deliberately does not include `chainId`, and the API requires `chainId` as a separate query parameter. The reason is that the cursor sorts on `(blockNumber, logIndex)`, and block numbers aren't comparable across chains — Polygon block 80M and Ethereum block 80M are unrelated points in time. Packing `chainId` into the cursor and adding it as a tiebreaker would let pagination "work" syntactically but produce a globally meaningless ordering. Cross-chain listings are a client-side concern: fan out, one request per chain, merge by `blockTimestamp` if needed.
...
**Decimal128 mirror for fee fields.** `integratorFee` and `lifiFee` are stored as strings so the full uint256 survives a round trip (`Number` would truncate above 2^53). That's the right canonical representation, but it forces every aggregation pipeline to `$toDecimal` per row before it can `$sum` or `$avg`. The plan is to add `integratorFeeDecimal` and `lifiFeeDecimal` as `Decimal128` mirrors, populated at insert time, so aggregation queries hit a typed numeric field directly. The string fields remain canonical. Decimal128's precision tops out at 34 significant decimal digits; anything larger leaves the mirror undefined and bumps a `fee_collector_fee_decimal_overflow_total{chain}` counter so callers can tell when an aggregate is under-counting.
